        ...
        <Link href={`/platform/profile/${Profile?.id}`} className="rounded-xl border-secondary bg-white border-opacity-20 p-8 flex gap-2 hover:shadow-xl duration-300 hover:scale-105 flex-col relative border-2 shadow-xl ">
            <div className="flex gap-4 items-top">
                ...
                <div className="flex flex-col">
                    ...
                    <h3 className="text-sm text-secondary text-opacity-50">{Profile?.currentRole}</h3>
                </div>
            ...
            {Profile.similarityScore && Profile.similarityScore > 0 && (<p className="text-complementary text-white absolute p-1 px-2 top-6 text-xs right-6 rounded-xl ml-auto bg-secondary">{Math.round(Profile?.similarityScore*100)}% Match</p>)}
        </Link>
    )
}