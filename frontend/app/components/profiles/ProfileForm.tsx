        ...
        <div className="grid grid-cols-2 gap-4 w-[80%] relative">
            { changes &&(<div className="absolute w-100% -top-10 w-full flex justify-center z-[1000] ">
                <div className=" fixed items-center  w-fit">
                    <div className="flex gap-24 bg-opacity-5 items-center justify-center border-2 shadow-xl rounded-md backdrop-blur-md p-4">
                        <label className="text-sm text-slate-500">You have unsaved changes</label>
                        {!saving ?(
                            <button onClick={()=> ProfileData ? updateProfileData() : generateProfile()} className="bg-secondary text-white p-2 rounded-md px-4 hover:opacity-50 duration-300 text-sm shadow-2xl">{ProfileData? "Save":"Create"}</button>    
                        ):(
            ...
            <div className="flex flex-col gap-4 ">
                <div className="flex items-center gap-4 bg-white rounded-md p-6 border-2">
                    <div className="w-fit h-fit relative">
                        ...
                        {Profile?.lastName?.length > 0 && (<h1 className="absolute top-0 bottom-0 left-0 right-0 my-auto h-fit mx-auto font-bold text-white w-fit text-md">{Profile?.firstName[0]} {Profile?.lastName[0]}</h1>)}
                    </div>
                    <div className="flex flex-col ">
                        <h1 className="font-bold text-md">{Profile?.firstName} {Profile?.lastName}</h1>
                        <p className="text-black text-opacity-50 text-sm">{Profile?.currentRole}</p>
                ...
                <div className="bg-white p-6 flex flex-col gap-4 rounded-md border-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="col-span-1 flex flex-col gap-2">
                            ...
                            <input placeholder="Enter first name" onChange={(e)=>setProfile({...Profile,["firstName"]:e.target.value})} defaultValue={Profile?.firstName} name="firstName" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 "></input>
                        </div>
                        <div className="col-span-1 flex flex-col gap-2">
                            <label className="text-sm text-black text-opacity-60 ">Last Name *</label>
                            <input placeholder="Enter last name" onChange={(e)=>setProfile({...Profile,["lastName"]:e.target.value})} defaultValue={Profile?.lastName} name="lastName" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 "></input>
                        </div>
                        
                        <div className="flex flex-col gap-2 col-span-2">
                            <label className="text-sm text-black text-opacity-60 ">Industry *</label>
                            <select onChange={(e)=>setProfile({...Profile,["industry"]:e.target.value})} defaultValue={Profile?.industry} name="industry" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 ">
                                {constants.industries.map((interests,index)=>{
                                    return(
                                        <option key={index} value={interests} className="p-1">{interests}</option>
                                    )
                                })}
                            </select>
                        </div>
                        <div className="flex flex-col gap-2 ">
                            ...
                            <select onChange={(e)=>setProfile({...Profile,["avaliability"]:e.target.value})} defaultValue={Profile?.avaliability} name="avaliabilty" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 ">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <select onChange={(e)=>setProfile({...Profile,["currentRole"]:e.target.value})} defaultValue={Profile?.currentRole} name="interests" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 ">
                                {constants.roles.map((role,index)=>{
                                    return(
                                        <option key={index} value={role} className="p-1">{role}</option>
                                    )
                                })}
                            </select>
                        </div>  
                ...
                <div className="bg-white p-6 flex flex-col gap-4 rounded-md border-2">
                    <h1 className="text-md font-bold">Links and Socials</h1>
                    <div className="grid grid-cols-1 gap-4">
                        <div className="col-span-1 flex flex-col gap-1">
                            <label className="text-sm text-black text-opacity-60 ">Github URL</label>
                            <input placeholder="github.com/username" onChange={(e)=>setProfile({...Profile,["githubUrl"]:e.target.value})} defaultValue={Profile?.githubUrl} name="firstName" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 "></input>
                        </div>
                        <div className="col-span-1 flex flex-col gap-1">
                            <label className="text-sm text-black text-opacity-60 ">LinkedIn URL</label>
                            <input placeholder="linkedin.com/" onChange={(e)=>setProfile({...Profile,["linkedInUrl"]:e.target.value})} defaultValue={Profile?.linkedInUrl} name="lastName" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 "></input>
                        </div>
                        <div className="col-span-1 flex flex-col gap-1">
                            <label className="text-sm text-black text-opacity-60 ">Twitter URL</label>
                            <input placeholder="x.com/username" onChange={(e)=>setProfile({...Profile,["twitterUrl"]:e.target.value})} defaultValue={Profile?.twitterUrl} name="lastName" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 "></input>
                        </div>
                        <div className="col-span-1 flex flex-col gap-1">
                            <label className="text-sm text-black text-opacity-60 ">Portfolio URL</label>
                            <input placeholder="Enter your portfolio url" onChange={(e)=>setProfile({...Profile,["portfolioUrl"]:e.target.value})} defaultValue={Profile?.portfolioUrl} name="lastName" className=" border-black border-opacity-15 rounded-md p-1 text-sm border-2 "></input>
                        </div>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-4 col-span-2 bg-white p-6 rounded-md border-2">
                    <label className="text-md font-bold ">Frameworks *</label>
                ...
                <div className="flex flex-col gap-4 col-span- bg-white p-6 rounded-md border-2">
                    <label className="font-bold text-md ">Languages *</label>
